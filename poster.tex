...
%			respect to $ Q $.
			...
			\item \structure{\textbf{KL-Divergence}} $ \kl{P}{Q} $ is the $ 
			f_{\mathrm{KL}} 
		...
		\item	\structure{\textbf{Squared Hellinger Distance}} $ H^2(P,Q) $ is 
		the $ \hel 
		$-divergence~\cite{daskalakis2017square}, where
			...
			(\sqrt{dP}-\sqrt{dQ})^2$.	
		\item  \structure{\textbf{Triangular Discrimination:}}
...
\begin{block}{3. Mutual Information Loss and Generalized JS 
		Divergence}
%		The mutual information loss arises naturally in many machine learning 
%		tasks, 
%		such as information-theoretic clustering~\citep{dhillon2003divisive} 
%		and 
%		categorical feature compression~\citep{bateni2019categorical}. 
	...
	%A labeled dataset can be modeled as a joint empirical distribution $\pr(X, 
	%C)$, where the random variable $X$ denotes the data item and $C$ denotes 
	%the label. 
