...
% 		Our reduction is based on the following observation.
 		\structure{\textbf{Reduction to MIPS:}} If 
...
% 		\cref{alg:krein-lsh}. 
		...
		\structure{\textbf{Apply LSH to MIPS}}
		As an example,  
% 		in \cref{ln:simple-lsh},
		we use \splsh 
% 		introduced by 
		\citep{neyshabur2015symmetric}. 
% 		Let us have a quick review of \splsh. 
		Assume $ 
		\|\bfx 
		\|^2_2\le M $
% 		that $ \cM\subseteq \bR^d $ is a finite set and  
		for all 
		$ 
		\bfx\in \cM $.
% 		there is a universal bound on the squared 2-norm, \ie, 
% 		\citet{neyshabur2015symmetric} assume that $ M=1 $ 
% 		without loss 
% 		of generality. We allow $ M $ to be any positive real number. 
		...
		Since 
		$\|L_1\|_2 = \|L_2\|_2 = M$,
% 		the norm of $ L_1 $ and $ L_2 $ is $ M $ and that therefore 
