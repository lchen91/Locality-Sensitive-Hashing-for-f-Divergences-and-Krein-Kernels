...
% 		Our reduction is based on the following observation.
 		\structure{\textbf{Observation:}} Suppose that $ K $ 
 		is a 
...
% 				\State Construct the atomic transform \[
% 				\tau(x,w,j)\triangleq 
% 				\left[\cos(w\ln(x))\sqrt{2x\int_{(j-1)\Delta}^{j\Delta} 
% 					\rho(w')dw'},\sin(w\ln(x))\sqrt{ 
...
% 				\begin{equation*}
% 				T_1(\bfx,M) \triangleq{} 
% 				[\eta_1,\sqrt{M-\|\eta_1(\bfx)\|^2_2},0],\quad T_2(\bfy,M) 
% 				\triangleq{} 
...
% 		\end{algorithm}
% 		
% 		We exemplify this technique by applying it to the MIL divergence.
% 		For ease of exposition, we define $ \rho(w)\triangleq \frac{2\sech(\pi 
% 			w)}{1+4w^2} $. The proposed approach \kr-LSH is presented in 
% 		\cref{alg:krein-lsh}. 
% 		To make the intuition of \eqref{eq:krein_inner_product} applicable in a 
% 		practical implementation, we have
% 		%The high-level idea is
% 		to truncate and discretize the 
% 		integral $ k(x,y) = \int_R \Phi_w(x)^* \Phi_w(y) 
% 		dw  $. First we analyze the truncation. The analysis is similar to 
